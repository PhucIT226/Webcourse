...
import { fetchLessons, deleteLesson } from "../../../redux/lessonSlice";
...
export default function LessonList() {
  ...
  const { data: lessons, pagination, loading, error } = useAppSelector(
    (state) => state.lesson
  );
        ...
        <table className="w-full text-sm text-left">
          <thead className="whitespace-nowrap bg-gray-100 text-gray-700 uppercase">
            <tr>
              ...
              >
                Tên bài học {renderSortIcon("position")}
              </th>
              <th className="border text-center px-4 py-3">Khóa học</th>
              <th
                className="border text-center px-4 py-3 cursor-pointer"
                onClick={() => handleSort("position")}
              >
                Thời lượng {renderSortIcon("position")}
              </th>
              <th className="border text-center px-4 py-3">Xem thử</th>
              <th
                className="border text-center px-4 py-3 cursor-pointer"
            ...
            {lessons.length > 0 ? (
              lessons.map((lesson: Lesson, index: number) => (
                ...
                >
                  ...
                  <td className="border px-4 py-2 font-medium truncate overflow-hidden
                                 whitespace-nowrap max-w-[200px]">
                      {lesson.title}
                  </td>
                  <td className="border px-4 py-2">{lesson.course?.title}</td>
                  <td className="border text-center px-4 py-2">{lesson.duration}</td>
                  <td className="border text-center px-4 py-2">
