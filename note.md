...
42. export const generateRefreshToken = async (userId, userService) => {
      - userService - là instance của service quản lý user, dùng để cập nhật refresh token vào database

43. exp - expiration time - là mốc thời gian token hết hạn, tính bằng giây kể từ thời điểm Unix epoch (01/01/1970)
...
9. Hook là gì? 
    ...
    - Một số hook phổ biến:
        ...
        | `useContext`  | Lấy dữ liệu từ React context.                                         |
        | `useMemo`     | Tối ưu hiệu năng, ghi nhớ giá trị tính toán.                          |
